local Postal = LibStub("AceAddon-3.0"):NewAddon("PostalEnhanced", "AceEvent-3.0", "AceHook-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("PostalEnhanced")
local ADDON_NAME = ... or "Postal"
_G["PostalEnhanced"] = Postal
...
StaticPopupDialogs["POSTAL_ENHANCED_DISABLE_POSTAL"] = {
	text = "Postal Enhanced detected that the original Postal addon is enabled. Disable Postal and reload the UI?",
	button1 = "Disable Postal",
	button2 = CANCEL,
	OnAccept = function()
		DisableAddOn("Postal")
		ReloadUI()
...
local function IsOriginalPostalEnabled()
	if ADDON_NAME == "Postal" or not GetAddOnInfo("Postal") then return end
	if GetAddOnEnableState then
		local state = GetAddOnEnableState(UnitName("player"), "Postal")
...
function Postal:OnInitialize()
	-- Version number
	if not self.version then self.version = GetAddOnMetadata(ADDON_NAME, "Version") end

	-- Initialize database
	if not PostalEnhancedDB and Postal3DB then
		PostalEnhancedDB = Postal3DB
	end
	self.db = LibStub("AceDB-3.0"):New("PostalEnhancedDB", defaults)
	...
	if IsOriginalPostalEnabled() then
		StaticPopup_Show("POSTAL_ENHANCED_DISABLE_POSTAL")
	end
...
function Postal:Print(...)
	local text = "|cff33ff99Postal Enhanced|r:"
	for i = 1, select("#", ...) do
...
function Postal.About()
	...
	wipe(t)
	tinsert(t, "|cFFFFCC00"..GetAddOnMetadata(ADDON_NAME, "Title").." v"..version.."|r")
	tinsert(t, "-----")
	tinsert(t, "")
	for name, module in Postal:IterateModules() do
