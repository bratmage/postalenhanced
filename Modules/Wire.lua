...
function Postal_Wire:onValueChangedFunc()
	...
	if subject == "" or subject:find(g) or subject:find(s) or subject:find(c) then
		...
		if money and money > 0 then
			...
			if GetLocale() == "ruRU" then
				if gold > 0 then
					SendMailSubjectEditBox:SetText(format("[%d+? %d+? %d+?]", gold, silver, copper))
				elseif silver > 0 then
					SendMailSubjectEditBox:SetText(format("[%d+? %d+?]", silver, copper))
				else
					SendMailSubjectEditBox:SetText(format("[%d+?]", copper))
				end
			else
				if gold > 0 then
